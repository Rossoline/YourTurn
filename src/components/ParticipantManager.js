...
import { addParticipant, toggleParticipant, updateParticipant, deleteParticipant } from "@/services/participantService";
...
export default function ParticipantManager({ supabase, familyId, participants, onUpdate }) {
  ...
  const handleSaveEdit = async () => {
    ...
    setLoading(true);
    await updateParticipant(supabase, editId, { name: editName.trim(), color: editColor });
    setEditId(null);
    setLoading(false);
    onUpdate();
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {participants.map((p) => {
        ...
        if (editId === p.id) {
          return (
            <div key={p.id} className="flex flex-col gap-3 p-4 bg-zinc-800 rounded-xl">
              <input
                type="text"
                value={editName}
              ...
              <div className="flex gap-2">
                {PALETTE.map((cl) => (
                  <button
                    key={cl.key}
                    onClick={() => setEditColor(cl.key)}
                    className={`w-8 h-8 rounded-full ${cl.dot} transition-all ${
                      editColor === cl.key ? "ring-2 ring-white scale-110" : "opacity-60 hover:opacity-100"
                    }`}
                  />
                ))}
              </div>
